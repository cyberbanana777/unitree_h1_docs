...
> 💡 Убедитесь, что у Вас установлен `git`. Если нет — установите через `sudo apt install git`.
...
Эти команды скачают папку с исходным кодом Unitree в ваш домашний каталог (`~`).
