...
# **Структура руководства по Unitree H1**
...
Unitree H1 поддерживает три основных способа управления, каждый из которых предназначен для своих задач и требует разного уровня подготовки.

#### 1. Пульт ДУ (Дистанционное управление)
...
#### 2. High-Level клиент (Команды верхнего уровня)
...
#### 3. Low-Level API (Прямое управление моторами)
...
*   **Как использовать:**
    ...
    *   **Для гибридного управления (только торс и руки):** Если робот уже находится в режиме ходьбы (управляется через High-Level API или пульт), можно независимо управлять корпусом и манипуляторами, отправляя сообщения типа `LowCmd` не в топик `/lowcmd`, а в специальный топик **`/arm_sdk`**. Это предотвращает конфликт команд при балансировке.
...
#### Контроль заряда батареи (отсутствие программной защиты).
...
<<--------------------------->>
Повторение
-#### Опасность выхода за пределы углов моторов.
...
-#### Важность правильного стартового положения.
-Робот инициализирует координаты при включении. Энкодеры на моторах относительные, т.е. положение, из которого Вы запускаете робота, должно быть вполне конкретным (указано на рисунке п. 1.3). Более подробную информацию можно найти [здесь](https://docs.quadruped.de/projects/h1/html/controller.html).
<<--------------------------->>
...
<<--------------------------->>
### 4.2. High-Level управление

**Доступные команды:** базовые движения (ходьба, повороты), смена поз, специальные действия. В стандартных примерах отсутствуют критически важные команды `Start` и `StopMove`, которые мы добавили в наших доработанных клиентах. 

**Наши разработки:** ROS2-пакет `high_level_control` с исправленным и расширенным API, включающим все необходимые команды для полноценного управления. (см. п. 2.2)

### 4.3. Low-Level управление

**Режим Develop** используется для прямого управления моторами. Обязательные условия:
- Активация только в подвешенном состоянии
- Полное отключение системных команд

**Вход/выход:** 
- Вход через SDK или пульт (L2+R2)
- Выход только программно или перезагрузкой

**Примеры управления:** отправка индивидуальных команд для каждого мотора через топик `/lowcmd` с указанием позиции. [unitree_h1_control_ws](https://github.com/cyberbanana777/unitree_h1_control_ws) (Сделать ссылку на пример без рук)
<<--------------------------->>

### 4.4. Гибридное управление (ходьба + руки)

**Управление через `/arm_sdk`:** позволяет управлять торсом и манипуляторами без перехода в Develop-режим, пока робот движется или балансирует.

**Ключевое требование:** сообщении есть неиспользуемое поле: поле мотора с индексом 9. Это поле используется для перехвата управления. Его нужно руками задавать на протяжении всего управления, т.е. передавать значение в это поле в каждом из сообщений. Если отослать сообщение без указания этого поля - руки робота примут позу, заданную системой, для балансирования.
**Примеры управления:**
[unitree_h1_control_ws](https://github.com/cyberbanana777/unitree_h1_control_ws) (Сделать ссылку на пример с руками) 

### 4.5. Наши разработки и библиотеки
...
## 8. Ссылки и ресурсы
###  **8.1. Официальные ресурсы Unitree:**
...
[Github unitree разработчиков](https://github.com/unitreerobotics)
