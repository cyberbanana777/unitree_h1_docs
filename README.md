...
- [**Руководство по Unitree H1**](#руководство-по-unitree-h1)
  - [Оглавление](#оглавление)
  - [1. 🚀Введение и быстрый старт](#1-введение-и-быстрый-старт)
    - [**1.1. О роботе Unitree H1:** Краткое описание возможностей и комплектации.](#11-о-роботе-unitree-h1-краткое-описание-возможностей-и-комплектации)
    - [**1.2. Цель данного руководства:** Что читатель найдет внутри.](#12-цель-данного-руководства-что-читатель-найдет-внутри)
    - [**1.3. Быстрый старт (шаги для первого включения):**](#13-быстрый-старт-шаги-для-первого-включения)
    - [Стартовое положение](#стартовое-положение)
    - [Включение робота и пульта ДУ](#включение-робота-и-пульта-ду)
    - [1. Запуск для движения с пульта](#1-запуск-для-движения-с-пульта)
    - [2. Запуск для программирования робота](#2-запуск-для-программирования-робота)
      - [**Режим работы**](#режим-работы)
      - [**Настройка сети для подключения**](#настройка-сети-для-подключения)
      - [Проверка связи](#проверка-связи)
    - [**Подключение по SSH для доступа к компьютеру робота:**](#подключение-по-ssh-для-доступа-к-компьютеру-робота)
  ...
  - [3. 🏗️ Основы архитектуры и управления](#3-️-основы-архитектуры-и-управления)
    - [3.1. Режимы работы робота](#31-режимы-работы-робота)
      - [Назначение каждого режима](#назначение-каждого-режима)
      - [Как переключаться между режимами](#как-переключаться-между-режимами)
    ...
    - [**3.3. Анатомия и компьютеры робота (ВАЖНОЕ ПРЕДУПРЕЖДЕНИЕ):**](#33-анатомия-и-компьютеры-робота-важное-предупреждение)
      - [**Адреса компьютеров Unitree H1:**](#адреса-компьютеров-unitree-h1)
      - [**!!Защита от дураков на исполняющем устройстве отсутствует!!**](#защита-от-дураков-на-исполняющем-устройстве-отсутствует)
    - [**3.4. Основные топики для управления и получения информации (DDS/ROS2):**](#34-основные-топики-для-управления-и-получения-информации-ddsros2)
  - [4. 👨‍💻 Практическое программирование под H1](#4--практическое-программирование-под-h1)
    - [**4.1. Предупреждения и меры безопасности:**](#41-предупреждения-и-меры-безопасности)
      - [**1. Контроль заряда батареи (отсутствие программной защиты).**](#1-контроль-заряда-батареи-отсутствие-программной-защиты)
      - [**2 . Опасность выхода за пределы углов моторов.**](#2--опасность-выхода-за-пределы-углов-моторов)
      - [**3. Важность правильного стартового положения.**](#3-важность-правильного-стартового-положения)
    - [**4.2. Структура топиков на роботе и работа с ними:**](#42-структура-топиков-на-роботе-и-работа-с-ними)
      - [**Основные топики:**](#основные-топики)
  ...
  - [5. 📡 Работа с сенсорами](#5--работа-с-сенсорами)
    ...
    - [**5.3. Камеры:** (В разработке)](#53-камеры-в-разработке)
    - [**5.4. Одометрия:** (В разработке)](#54-одометрия-в-разработке)
  - [6. 👁️ Визуализация и наши репозитории](#6-️-визуализация-и-наши-репозитории)
  ...
  - [7. 🔧 Решение проблем (Troubleshooting)](#7--решение-проблем-troubleshooting)
    ...
    - [**7.3. Не видны топики с робота:**](#73-не-видны-топики-с-робота)
...
*   **L1 + A:** Войти в режим `Damping` (Робот висит, его ноги могут касаться пола, но чуть-чуть).
...
*   **R2 + X:** Активировать `Sports Mode`, чтобы перейти к балансированию и ходьбе с пульта. 
...
*   Соедините компьютер и робот кабелем Ethernet.
...
1.  **Запустите пример (на стороне пользователя):**
...
> ⚠️ **Если топики не повились** при запуске любой из команд, то посмотрите решение в разделе [Решение проблем](https://github.com/cyberbanana777/unitree_h1_docs?tab=readme-ov-file#7--решение-проблем-troubleshooting).
...
Мы также переработали его, добавили несколько дополнительных функций и сделали по его красивее. Получилось 2 варианта исполнения. Подробнее Вы можете рассмотреть ROS2-пакет `high_level_control` в [данном репозитории](https://github.com/cyberbanana777/unitree_h1_control_ws)
...
*   **Назначение:** Идеален для базового контроля, первоначальной настройки, тестирования и обеспечения безопасности (аварийная остановка — **`L2 + B`**). Является обязательным инструментом для безопасного запуска и перевода робота в нужные режимы перед программным управлением.

###   **3.3. Анатомия и компьютеры робота (ВАЖНОЕ ПРЕДУПРЕЖДЕНИЕ):**
Из оффициальной документации [соответствие моторов и их индексов](https://support.unitree.com/home/en/H1_developer/Joint_motor_sequence).
...
1. **Классический способ через команды ROS2.** Этот метод требует, чтобы предварительно были засуршены (`source unitree_ros2/cyclonedds_ws/intsall/local_setup.bash`) типы сообщений, которые необходимо предварительно скомпилировать (см. гайд по установке и компиляции в [репозитории](https://github.com/unitreerobotics/unitree_ros2/tree/master)). Пример: `ros2 topic echo /lowstate`
...
**Важный нюанс:** Стандартный пример клиента от Unitree не полный. В нём отсутствуют ключевые команды `Sport` (для перехода в режим балансировки) и `StopMove` (для остановки движения). Работающий пример High_level клиента см. п. 2.2.
...
1. **Робот дёргается при отправке команд.**
	**Причина:** Конфликт ваших команд с системными командами контроллера. Вероятнее всего вы перевели робоота в один из режимов: `Preparation` или `Sport`, и отправляете команды в топик `/lowcmd`. Или отпрвляете команды через `/lowcmd` в `Damping` режиме (в этом режиме моторы находятся в состоянии демфирования - сопротивляются изменению положения).
     * Для управления верхними конечносятми робота и торсом в режимах `Preparation` или `Sport`, отправляйте команды в топик `/arm_sdk`.  
...
2. **Робот работ не двигается при отправке команд.**
	...
	**Решение:** 
     ...
     * Чтобы управлять семи конечностями робота командами в `/lowcmd` нужно перевести робта в режим `Develop`.
